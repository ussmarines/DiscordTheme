...
- `src/*.css` — theme source, split by area. Index:
  ...
  - `colors.css` — overrides of Discord's `--*` design-system variables, mapped to sibnight palette. Most "wrong color" bugs are fixed here.
...
- `themes/sibnight.theme.css` — user-facing entry: variables + `@import` of the build.
- `build/sibnight.css` — generated from `src/*.css`. **Don't hand-edit. Users import the theme from here.**
- `scripts/dev.js` — watcher for desktop Vencord (`npm run dev`).
- `scripts/serve.js` + `scripts/inject.js` + `scripts/sibnight-dev.user.js` — browser dev flow. See `BROWSER_DEV.md`.

## Hard rules

- Edit `src/*.css`, never `build/sibnight.css`.
- User-facing CSS variables (colors, sizes, toggles) live in `themes/sibnight.theme.css`. Structural rules live in `src/*.css`.
